...
Zero dependencies (native `fetch`, Node 18+). ESM only (`"type": "module"`).
...
> instead of plain-object access (still `result.content` in JS either
> way, but now a class instance, not a bare object literal — `instanceof`
> checks and `.raw` are new). Every object carries the full original
> response as `.raw`, so nothing is lost. Discovery calls (`capabilities`,
> `status`, `models`, `usage`) and CRUD metadata (agent-configs,
> flow-definitions) are unaffected — those still return plain objects.
> The package entry point also moved from `src/FireClient.js` to
> `src/index.js` (re-exports everything) — `import { FireClient } from
> '@moduspromethean/fire-sdk'` is unaffected either way.
...
```js
const caps = await client.capabilities();   // no token required, still a plain object
...
console.log(caps.account_setup);            // { url: ".../portal", ... } — where a human sets up billing
```
...
const result = await client.chat(
  ...
  { systemPrompt: 'Be concise.', speciesName: 'claude-sonnet-4-5', temperature: 0.7 }, // or use Species, below
);
console.log(result.content, result.priceUsd);

const parallel = await client.chatParallel([
...
for (const item of parallel.results) {
  console.log(item.ok ? item.content : item.error);
}
```
...
```js
const result = await client.image('a lighthouse at dawn, painterly americana style', { n: 1, size: '1024x1024' });
console.log(result.images[0].url ?? result.images[0].b64, result.priceUsd);
```

## Species — the model taxonomy, as a navigable registry

Fire organizes every model under a biological taxonomy (family → genus →
species). `Species` is a **generated** object (`scripts/generate_species.py`,
run against `GET /v1/capabilities`, checked in — not fetched at import
time) that turns that taxonomy into a discoverable, typo-proof namespace:

```js
import { Species } from '@moduspromethean/fire-sdk/species';

await client.chat([...], { speciesName: Species.anthropic.CLAUDE_SONNET_4_5 });
await client.image('...', { speciesName: Species.openai.GPT_IMAGE_1 });
```
...
```js
const result = await client.runWorkflow('article', { source_text: '...', seed: '...' });
console.log(result.workflow);   // "article"
console.log(result.raw);        // the workflow's actual (workflow-specific) response
```
...
```bash
FIRE_TOKEN=fire_sk_... node examples/triad_example.js "Should remote work be the default?"
```
...
await run.wait();   // mutates `run` in place

if (run.status === 'awaiting_human') {
  const gate = run.gatingStep();
  console.log(gate.promptForHuman);
  console.log('Agreeable:', gate.context.hot_1);
  console.log('Contrarian:', gate.context.hot_2);

  await run.resume(gate.stepKey, { note: 'your guidance' });
  await run.wait();
}

console.log(run.output.content);   // the cool synthesizer's final answer — still a plain object, output shape varies
console.log(run.totalCostUsd);     // real, billed cost across all 3 agent calls
```
...
```js
await client.createAgentConfig('my-hot-agent', 'My Hot Agent', {
  speciesName: Species.anthropic.CLAUDE_SONNET_4_5, systemPrompt: '...', temperature: 0.9,
});
```
...
const run = await client.runFlow({ flow: spec, input: { question: '...' } });
```
...
visibility. Pass `{ isPublic: false }` to `createFlowDefinition` to keep
it private instead. External (paying) accounts are capped at 10 custom
flow definitions by default; deactivating one frees a slot.
...
/ `{{step_key.human_input.*}}`. See `examples/triad_example.js` and
`src/FireClient.js`'s JSDoc for the full shape — or just inspect
`client.getFlowDefinition('triad')` to see a real one (still a plain
object — flow *definitions* are metadata/CRUD, not a call result).
...
```js
const run = await client.getFlowRun(runId, { verbosity: 'compact' });
...
What *did* seem worth doing: typed results instead of bare objects, so
`instanceof ChatResult` and a fixed set of documented fields beat an
untyped blob. But L1 (`ChatResult`/`ImageResult`), L2 (`WorkflowResult`),
and v2 (`FlowRun`) are three separate root classes, not one hierarchy —
...
Regenerate `src/species.js` (and the Python/PHP equivalents) after a
